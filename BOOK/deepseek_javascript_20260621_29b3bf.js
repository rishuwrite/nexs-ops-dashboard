javascript:(async () => {
  ...
  const PRINT_CONCURRENCY = 250;
  const CREDENTIALS = { userName: "MY_USERNAME", password: "MY_PASSWORD" };
  ...
  // ✅ FIXED: Complete autoLogin with logout and proper cookie handling
  async function autoLogin() {
    setStatus("Session expired. Logging out existing session...", "#ffd866");
    const currentDateTime = new Date().toISOString().replace("T", " ").substring(0, 19);
    
    // STEP 1: Logout to clear existing session
    try {
      await fetch("https://app.nexs.lenskart.com/v1/user/logout", {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      // Network error - non-fatal, continue anyway
    }
    
    // Small pause to let server clear the session
    await new Promise(r => setTimeout(r, 500));

    // STEP 2: Login with proper credentials:include
    setStatus("Logging in...", "#ffd866");
    const loginResponse = await fetch("https://app.nexs.lenskart.com/v1/user/login", {
      method: "POST",
      credentials: "include", // ✅ FIXED: Added this
      headers: {
    ...
    // STEP 3: AuthMe - browser has cookie, no Authorization header needed
    setStatus("Establishing session...", "#ffd866");
    const authMeResponse = await fetch("https://app.nexs.lenskart.com/v1/user/authme", {
      ...
      headers: {
        ...
        "workstation-id": "",
        "source-domain": "https://app.nexs.lenskart.com"
        // ✅ FIXED: Removed Authorization header - rely on cookie
      }
    });
    ...
    setStatus("✅ Re-logged in! Resuming...", "#7ee787");
  }
  ...
  async function fetchManifestQuery(query) {
    const res = await fetch(`https://app.nexs.lenskart.com/nexs/manifest/api/v1/fetch/filter?page=0&size=150&sort=createdAt,desc&shippingProvider=${encodeURIComponent(query)}`, {
      method: "GET",
      credentials: "include",
      headers: {
        accept: "application/json",
  ...
  async function fetchManifestDone() {
    ...
    for (const courier of COURIERS) {
      ...
      for (const query of queries) {
        requests.push(
          fetchManifestQuery(query).then(rows => ({ courier, rows }))
        );
      }
    }
    const results = await Promise.all(requests);
    results.forEach(({ courier, rows }) => {
      rows.forEach(item => {
