javascript:(async () => {
  ...
  async function autoLogin() {
    ...
    setStatus("Logging out existing session...", "#ffd866");
    try {
    ...
    if (!loginResponse.ok || !loginData.success) {
      throw new Error("LOGIN_FAILED: " + (loginData.message || "unknown"));
    }
    ...
    if (!authMeData.success) {
      throw new Error("AUTHME_FAILED: " + (authMeData.errors?.[0]?.errorMessage || "unknown"));
    }
  ...
  async function fetchManifestQuery(query) {
    if (isSessionDead) throw new Error("SESSION_EXPIRED");
